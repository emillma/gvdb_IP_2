...
def create_binary_image(im):
    ...
    im_freq = np.abs(np.fft.fftshift(np.fft.fft2(im)))
    limit = 0.5  # percent of pixels
    limit_val = im_freq.ravel()[np.argsort(
        im_freq.ravel())[int(round(im_freq.size*(100-limit)/100))]]

    binary_im = im_freq > limit_val
